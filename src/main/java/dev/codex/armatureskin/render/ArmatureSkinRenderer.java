...
public final class ArmatureSkinRenderer {
    ...
    public boolean renderPlayer(AbstractClientPlayer player, float yaw, float tickDelta, PoseStack matrices, MultiBufferSource buffers, int light) {
        ...
        for (ArmatureModel.Mesh mesh : current.meshes()) {
            ...
            for (int i = 0; i + 2 < indices.length; i += 3) {
                emitVertex(consumer, entry, meshVertices.get(indices[i]), skinMatrices, light);
                emitVertex(consumer, entry, meshVertices.get(indices[i + 1]), skinMatrices, light);
                emitVertex(consumer, entry, meshVertices.get(indices[i + 2]), skinMatrices, light);
            }
        }
    ...
    private void emitVertex(VertexConsumer consumer, PoseStack.Pose entry, ArmatureModel.Vertex vertex, Matrix4f[] skinMatrices, int light) {
        Vector3f position = skin(vertex, skinMatrices);
